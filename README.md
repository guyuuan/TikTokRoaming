...
push 任意分支时会构建 daily APK，push 任意 tag 时会构建正式 APK。也可以在 Actions 页面手动运行 `Build Android APK`，输入目标 Release tag（例如 `v1.2.0`）并选择是否标记为 prerelease；工作流会构建同版本的签名 APK，并创建或更新对应的 GitHub Release。GitHub Release 只公开附加 APK，R8 `mapping.txt` 仍作为 Actions 产物保留。

所有构建完成后都会分别直接上传 APK 和 R8 `mapping.txt`，不会将它们打包成 ZIP。请在仓库的 Actions secrets 中配置：
